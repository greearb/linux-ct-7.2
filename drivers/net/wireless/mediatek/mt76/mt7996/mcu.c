			  ...
			  struct sk_buff *skb, int seq)
{
	...
	if (!skb) {
		...
		else if (!test_and_set_bit(MT76_MCU_RESET, &dev->mphy.state)) {
			    ...
			    struct all_sta_trx_rate *mcu_rate)
{
...

	{
		struct mt7996_phy *mphy =
			mt7996_vif_link_phy(&msta_link->sta->vif->deflink);
		if (!mphy)
			return 0;
			     ...
			     const char *fw_file, enum mt7996_ram_type ram_type)
{
	...
	strscpy(dev->mt76.fw.wm_fw_ver, hdr->fw_ver, 10);
	strscpy(dev->mt76.fw.wm_build_date, hdr->build_date, 15);
...
#define EML_OMN_CONTROL_EMLSR_MODE	0x01
	...
	if (eml_omn->control & EML_OMN_CONTROL_EMLSR_MODE) {
		...
		for_each_set_bit(linkid, &bitmap, IEEE80211_MLD_MAX_NUM_LINKS) {
			...
			phy = mt7996_vif_link_phy(mconf_link);

			eml_op->link_bitmap |= BIT(phy->mt76->band_idx);
		}
	}
	...
	mt76_dbg(&dev->mt76, MT76_DBG_MLD,
		 ...
		 !!(eml_omn->control & EML_OMN_CONTROL_EMLSR_MODE), eml_op->link_bitmap);
