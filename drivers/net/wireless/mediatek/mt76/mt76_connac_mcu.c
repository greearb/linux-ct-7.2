			  ...
			  const char *fw_wa)
{
	...
	strscpy(dev->fw.wm_fw_ver, hdr->fw_ver, 10);
	strscpy(dev->fw.wm_build_date, hdr->build_date, 15);
	...
	dev_info(dev->dev, "WA Firmware Version: %.10s, Build Time: %.15s",
		 hdr->fw_ver, hdr->build_date);
	strscpy(dev->fw.wa_fw_ver, hdr->fw_ver, 10);
	strscpy(dev->fw.wa_build_date, hdr->build_date, 15);
...
int mt76_connac2_load_patch(struct mt76_dev *dev, const char *fw_name)
{
	...
	strscpy(dev->fw.build_date, hdr->build_date, 15);
