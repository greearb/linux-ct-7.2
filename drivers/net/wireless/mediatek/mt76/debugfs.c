...
mt76_version(struct seq_file *s, void *data)
{
	...
	/* Add specific connac2 radios here */
		...
		seq_printf(s, "7986\n");
	else if (is_mt7990(dev))
		seq_printf(s, "7990\n");
	else if (is_mt7992(dev))
		seq_printf(s, "7992\n");
			   ...
			   const struct file_operations *ops)
{
	...
	debugfs_create_devm_seqfile(dev->dev, "active-tokens", dir,
				    mt76_active_tokens_read);
	debugfs_create_u8("block_traffic", 0600, dir, &dev->block_traffic);
