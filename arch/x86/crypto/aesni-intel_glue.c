...
static int compute_mac(struct crypto_aes_ctx *ctx, struct aes_key *key,
		       u8 mac[], u8 *data, int n,
				   ...
				   bool do_simd)
{
	...
	while (len) {
		...
		ilen = compute_mac(ctx, key, mac, walk.addr, n,
				   ilen, idata, do_simd);
		len -= n;
...
static int aesni_ccm_encrypt(struct aead_request *req)
{
	...
	struct aes_key *key = crypto_tfm_ctx(&aead->base);
	...
	if (req->assoclen)
		ccm_calculate_auth_mac(req, ctx, key, mac, req->src, do_simd);
