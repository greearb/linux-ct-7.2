					    ...
					    unsigned long *next_delay)
{
	...
	sdata = rcu_dereference_protected(local->scan_sdata,
					  lockdep_is_held(&local->hw.wiphy->mtx));
	if ((sdata->pr_conf.flags & CT_PREQ_DISABLE_HT))
		flags |= IEEE80211_PROBE_FLAG_DISABLE_HT;
	if ((sdata->pr_conf.flags & CT_PREQ_DISABLE_VHT))
		flags |= IEEE80211_PROBE_FLAG_DISABLE_VHT;
	if ((sdata->pr_conf.flags & CT_PREQ_DISABLE_HE))
	...
	if (sdata->pr_conf.flags & CT_PREQ_DISABLE_320)
		flags |= IEEE80211_PROBE_FLAG_DISABLE_320;
