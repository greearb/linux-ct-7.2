...
static int pg_populate_report(struct pktgen_dev_report* rpt, struct pktgen_dev* pkt_dev) {
	...
	strscpy(rpt->thread_name, pkt_dev->pg_thread->tsk->comm, 32);
	strscpy(rpt->interface_name, pkt_dev->ifname, 32);
	...
	strscpy(rpt->dst_min, pkt_dev->dst_min, IP_NAME_SZ);
	strscpy(rpt->dst_max, pkt_dev->dst_max, IP_NAME_SZ);
	strscpy(rpt->src_min, pkt_dev->src_min, IP_NAME_SZ);
	strscpy(rpt->src_max, pkt_dev->src_max, IP_NAME_SZ);
...
static long pktgen_proc_ioctl(struct file* file, unsigned int cmd,
			      unsigned long arg) {
        int err = 0;
...
#ifdef USE_NQW_CALLBACK
/* Runs from interrupt */
static int pg_notify_queue_woken(struct net_device* dev) {
...
static int pktgen_add_device(struct pktgen_thread *t, const char *ifname)
{
	...
	pkt_dev->burst = 1;
	pkt_dev->node = -1;
	strscpy(pkt_dev->ifname, ifname, sizeof(pkt_dev->ifname));
